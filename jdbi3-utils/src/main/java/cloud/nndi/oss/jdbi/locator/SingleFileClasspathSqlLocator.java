...
/**
 * Behaves like @{@link org.jdbi.v3.core.locator.ClasspathSqlLocator} but allows
 * you to put all your queries in one file. A bit like <a href="https://github.com/knadh/goyesql">goyesql</a>
 * <br/>
 * Locates SQL in {@code .sql} files on the classpath.  Given a class and
 * method name, for example {@code com.foo.Bar#query}, load a
 * classpath resource name like {@code com/foo/Bar.sql}.
 * <br/>
 * The locator will then look for a query prefixed with commend {@code -- name: query}
 * and the actual query in the SQL file.
 *
 * The contents are then parsed, cached, and returned for use by a statement.
 */
public class SingleFileClasspathSqlLocator {

    private static final String SQL_EXTENSION = "sql";
    private static final SqlQueriesFileParser READER = new SqlQueriesFileParser();
    ...
    private SingleFileClasspathSqlLocator(CheckedFunction<InputStream, String> parser) {
     ...
     */
    public static SingleFileClasspathSqlLocator removingComments() {
        final SqlScriptParser commentStripper =
                new SqlScriptParser((t, sb) -> sb.append(t.getText()));
        return new SingleFileClasspathSqlLocator(
     ...
     */
    public static SingleFileClasspathSqlLocator create() {
        return new SingleFileClasspathSqlLocator(SingleFileClasspathSqlLocator::readAsString);
    }
     ...
     * a resource named <code>com/foo/Bar.sql</code> on the classpath and returns its contents as a String.
     *
    ...
    private static ClassLoader selectClassLoader() {
        return Optional.ofNullable(Thread.currentThread().getContextClassLoader())
                .orElseGet(SingleFileClasspathSqlLocator.class::getClassLoader);
    }
